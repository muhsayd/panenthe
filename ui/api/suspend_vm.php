...
if($vo_vps->get_is_suspended() == '0'){
	func_vps::suspension($vo_vps,'1');
	$vpsDriver->suspend(func_vps_ip::get_all_ips($vo_vps));
	$action = 'suspended';
	$action_pl = 'suspension';
}
else
{
	func_vps::suspension($vo_vps,'0');
	$vpsDriver->unsuspend(func_vps_ip::get_all_ips($vo_vps));
	$action = 'unsuspended';
