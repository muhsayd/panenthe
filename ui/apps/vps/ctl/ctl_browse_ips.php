...
class ctl_browse_ips {
	...
	private function browse_action(){
		...
		if(isset(dev::$get['remove_ip'])){
			func_vps_ip::remove_ip();
		}

		if(isset(dev::$post['browse_action_delete'])){
			func_vps_ip::browse_action_delete();
		}
	}
	...
	private function pagination(){
		...
		if(!isset(dev::$get['search_words'])){
			$base_url = main::$url.'/index.php?app=vps&sec=browse_ips&vps_id='.dev::$get['vps_id'];
			$page_url = main::$url.'/index.php?app=vps&sec=browse_ips&vps_id='.dev::$get['vps_id'].'&page_no={page_no}';
		}
		else
		{
			$base_url = main::$url.'/index.php?app=vps&sec=browse_ips&vps_id='.dev::$get['vps_id'].'&search_words='.dev::$get['search_words'].'&search_col='.dev::$get['search_col'];
			$page_url = main::$url.'/index.php?app=vps&sec=browse_ips&vps_id='.dev::$get['vps_id'].'&search_words='.dev::$get['search_words'].'&search_col='.dev::$get['search_col'].'&page_no={page_no}';
		}
	...
	private function get_ips(){
		...
		$this->pages = func_vps_ip::get_ips($this->vo_vps,dev_search::get_sql(),dev_search::get_sql_value(),dev_pagination::get_sql());
		
	}

	private function show_page(){

		vw_ips::load_browse_js();
		vw_ips::title('Browse VM Ip addresses for VPS ID: '.dev::$get['vps_id']);
		vw_ips::browse($this->pages,dev_pagination::get_page_html());
	}
